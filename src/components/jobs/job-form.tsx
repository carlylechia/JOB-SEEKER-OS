...
export function JobForm({
  ...
  async function handleDelete() {
    if (!onDelete) return;
    const confirmed = window.confirm('Archive/delete this job lead? This cannot be undone.');
    if (!confirmed) return;
    setIsDeleting(true);
    setError(null);
    try {
        ...
        <div className="space-y-6">
          <div className="card-pad">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              ...
              <div className="md:col-span-2"><label className="muted">Role title</label><input className="input mt-1" value={values.title} onChange={(e) => update('title', e.target.value)} required /></div>
          ...
          <div className="card-pad">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <div>
                ...
                <select className="select mt-1" value={values.status} onChange={(e) => update('status', e.target.value as JobFormValues['status'])}>
                  {statuses.map((status) => <option key={status} value={status}>{status}</option>)}
                </select>
              </div>
              <div>
                <label className="muted">Next follow-up</label>
                <input className="input mt-1" type="date" value={values.nextFollowUp} onChange={(e) => update('nextFollowUp', e.target.value)} />
              </div>
              <div>
                <label className="muted">Minimum salary</label>
                <input className="input mt-1" type="number" min="0" value={values.salaryMin} onChange={(e) => update('salaryMin', e.target.value)} />
              </div>
              <div>
                <label className="muted">Maximum salary</label>
                <input className="input mt-1" type="number" min="0" value={values.salaryMax} onChange={(e) => update('salaryMax', e.target.value)} />
              </div>
              <div>
                <label className="muted">Currency</label>
                <input className="input mt-1" value={values.currency} onChange={(e) => update('currency', e.target.value.toUpperCase())} maxLength={6} />
              </div>
            </div>
            <div className="mt-4">
              <label className="muted">Notes</label>
              <textarea className="input mt-1 min-h-32" value={values.notes} onChange={(e) => update('notes', e.target.value)} placeholder="Why this role, risks, referral notes, stack clues, or tailoring reminders..." />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="card-pad">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="muted mt-1">Use the 0–5 rubric to score fit before or after saving.</p>
              </div>
              <div className="rounded-xl border border-line bg-white/5 px-3 py-2 text-sm text-muted">Avg signal: <span className="text-white">{averageSignal}/5</span></div>
            </div>

            <div className="mt-4 space-y-3">
              {scoreFields.map((field) => (
                <div key={field.key} className="rounded-xl border border-line p-3">
          ...
          <div className="card-pad space-y-3">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {mode === 'edit' && onDelete ? <button type="button" className="btn-secondary border-red-400/20 text-red-200" disabled={isDeleting} onClick={handleDelete}>{isDeleting ? 'Deleting...' : 'Delete lead'}</button> : null}
            </div>
          </div>
        </div>
      </div>
