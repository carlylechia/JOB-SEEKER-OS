...
export async function PATCH(request: Request, { params }: Params) {
  ...
  const rate = applyRateLimit(
    `jobs:update:${session.user.id}:${getRequestIp(request)}`,
    30,
    60_000,
  );
  ...
  try {
    ...
    const job = buildStoredJobPayload(
      {
        ...parsed.data,
        id,
        ...
        prepPack: existing.prepPack,
        dateFound: existing.dateFound.toISOString().slice(0, 10),
      },
      preferences,
    );
...
export async function DELETE(request: Request, { params }: Params) {
  ...
  const { id } = await params;
  const rate = applyRateLimit(
    `jobs:delete:${session.user.id}:${getRequestIp(request)}`,
    10,
    60_000,
  );
  if (!rate.ok) return jsonError("Too many requests", 429, undefined, request);
