...
export async function GET(request: Request, { params }: Params) {
  ...
  try {
    ...
    await logImportantError({ event: 'job_get_failed', userId: session.user.id, jobId: id, route: `/api/jobs/${id}`, error, context: { method: 'GET' } });
    return jsonError('Unable to fetch job lead.', 500, undefined, request);
  }
}
...
export async function DELETE(request: Request, { params }: Params) {
  ...
  try {
    ...
    await logImportantError({ event: 'job_delete_failed', userId: session.user.id, jobId: id, route: `/api/jobs/${id}`, error, context: { method: 'DELETE' } });
