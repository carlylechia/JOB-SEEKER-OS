...
export default function JobDetailsPage() {
  ...
  const { getJob, updateJob, deleteJob, isLoading } = useJobs();
  ...
  async function handleUpdate(values: JobFormValues) {
    job && (await updateJob(job.id, values));
    setIsEditing(false);
  }

  async function handleDelete() {
    job && (await deleteJob(job.id));
    router.push('/jobs');
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={
          <div className="flex gap-3">
            {job.jobUrl ? <Link className="btn-secondary" href={job.jobUrl} target="_blank">Open listing</Link> : null}
            <button className="btn-primary" type="button" onClick={() => setIsEditing((prev) => !prev)}>
      ...
      {isEditing ? (
        <JobForm mode="edit" job={job} onSubmit={handleUpdate} onDelete={handleDelete} />
      ) : (
          ...
          <div className="space-y-6">
            <div className="card-pad">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <div><div className="muted">Compensation</div><div>{job.salaryMin || '—'}{job.salaryMax ? ` - ${job.salaryMax}` : ''} {job.currency}</div></div>
                <div><div className="muted">Date found</div><div>{job.dateFound}</div></div>
                <div><div className="muted">Next follow-up</div><div>{job.nextFollowUp || '—'}</div></div>
                <div><div className="muted">Status</div><div>{job.status}</div></div>
              </div>
            </div>
            <div className="card-pad">
              <h3 className="text-lg font-semibold">Notes</h3>
              <p className="mt-3 whitespace-pre-wrap text-muted">{job.notes || 'No notes yet.'}</p>
            </div>
            <div className="card-pad">
              ...
              <div className="mt-4 space-y-3">
                {job.contacts.length ? job.contacts.map((contact) => (
                  <div key={contact.id} className="rounded-xl border border-line p-3">
                    ...
                    {contact.notes ? <div className="mt-2 text-sm text-muted">{contact.notes}</div> : null}
                  </div>
